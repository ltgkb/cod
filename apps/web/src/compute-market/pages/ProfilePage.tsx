...
import { ChatCircleDots, IdentificationCard, MapPin, Package, SealCheck, Storefront, Ticket, UserCircle, Users } from '@phosphor-icons/react';
...
export function ProfilePage({ api, navigate, signedIn, requireLogin, capabilities, displayName }: ComputePageProps & { capabilities: ComputeCapabilities; displayName: string }) {
  const resource = useComputeResource(async (signal) => {
    if (!signedIn) return { assets: null, devices: [] };
    const [assets, devicePage] = await Promise.all([
    ...
    return { assets, devices: devicePage.items };
  }, [signedIn, capabilities.assets, capabilities.devices]);
