...
export function ComputeSideNav({ path, capabilities, navigate, onExit }: { path: string; capabilities: ComputeCapabilities; navigate: (path: string) => void; onExit: () => void }) {
  ...
  const items = [
    ...
    { path: '/compute/hosting', label: '设备托管', icon: Cpu, visible: capabilities.hosting }, { path: '/compute/orders', label: '我的订单', icon: Package, visible: true },
    ...
    { path: '/compute/me', label: '我的', icon: UserCircle, visible: true },
  ];
