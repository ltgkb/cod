...
export function OfferCard({ offer, onOpen }: { offer: ComputeOfferV2; onOpen: (offer: ComputeOfferV2) => void }) {
  ...
  return <article className="compute-offer-card" tabIndex={0} role="link" aria-label={`查看 ${offer.title} 详情`} onClick={() => onOpen(offer)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); onOpen(offer); } }}>
    ...
    <div className="compute-offer-body"><h3>{offer.title}</h3><div className="compute-price-band"><strong>{formatCardHours(sku?.priceCardHoursMilli)}</strong><span>{sku?.priceCardHoursMilli === null || sku?.priceCardHoursMilli === undefined ? `/${sku ? periodLabel(sku.period) : '小时'}` : `卡时/${sku ? periodLabel(sku.period) : '小时'}`}</span>{discounted && <del>{formatCardHours(sku.compareAtPriceCardHoursMilli)}卡时</del>}</div>
