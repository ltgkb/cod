...
import type { AdminComputeRequestSummary, DeviceRecord, KnowledgeHit, ProductManifest, TaskStatus, WorkspaceFile } from '@cod/contracts';
import {
  cancelRemoteTask,
  createRemoteTask,
...
type Overlay = 'login' | 'new-task' | 'account' | 'commands' | 'models' | 'compute' | 'compute-admin' | 'mobile-menu' | null;
...
const initialComputeDraft: ComputeDraft = {
  tab: 'rental', offerId: '', company: '', contactName: '', contactPhone: '', city: '', gpuModel: 'NVIDIA H100 80GB', quantity: 1,
  ...
  return <div className="login-form">
    ...
    <form onSubmit={submit}>
      {mode === 'login' && <>
        ...
        <label>密码<input aria-label="密码" name="loginPassword" type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" minLength={10} maxLength={128} required /></label>
      </>}
      ...
      {registering && !nativeRegistration && registrationStep === 'password' && <>
        ...
        <label>设置密码<input aria-label="密码" name="newPassword" type="password" value={password} onChange={(event) => updateFinalCredential(() => setPassword(event.target.value))} autoComplete="new-password" minLength={10} maxLength={128} required autoFocus /></label>
        <label>确认密码<input aria-label="确认密码" name="confirmPassword" type="password" value={confirmPassword} onChange={(event) => updateFinalCredential(() => setConfirmPassword(event.target.value))} autoComplete="new-password" minLength={10} maxLength={128} required /></label>
        ...
        <p className="password-hint">密码须为 10-128 位，并同时包含字母和数字。邀请关系注册后不可自行更改。</p>
      </>}
...
function ComputeMarket({ offers, requests, signedIn, draft, onDraftChange, onLogin, onSubmit }: { offers: ComputeOffer[]; requests: ComputeRequest[]; signedIn: boolean; draft: ComputeDraft; onDraftChange: (draft: ComputeDraft) => void; onLogin: () => void; onSubmit: (input: ComputeRequestInput) => Promise<void> }) {
  ...
  const [error, setError] = useState('');
  const {
    tab, offerId, company, contactName, contactPhone, city, gpuModel, quantity, durationHours, termMonths, requirements,
  ...
  return <div className="compute-market">
    ...
    {requests.length > 0 && <section className="compute-requests"><header><strong>我的需求</strong><small>商务确认后状态会更新</small></header>{requests.map((request) => <div key={request.id}><span><strong>{requestKindLabel[request.kind]} · {request.gpuModel}</strong><small>{request.quantity} 卡{request.durationHours ? ` · ${request.durationHours} 卡时` : ''}{request.termMonths ? ` · ${request.termMonths} 个月` : ''}{request.hostingPeriodMonths ? ` · 托管 ${request.hostingPeriodMonths} 个月` : ''} · {formatTime(request.createdAt)}</small></span><i className={request.status}>{statusLabel[request.status]}</i></div>)}</section>}
  </div>;
}
...
const computeStatusLabels: Record<ComputeRequest['status'], string> = {
  submitted: '已提交', contacting: '联系中', quoted: '已报价', closed: '已关闭',
};
const computeStatusTransitions: Record<ComputeRequest['status'], ComputeRequest['status'][]> = {
  submitted: ['contacting', 'closed'], contacting: ['quoted', 'closed'], quoted: ['closed'], closed: [],
};
...
function AdminComputeRequestDetail({ request, onUpdate }: { request: ComputeRequest; onUpdate: (id: string, status: ComputeRequest['status'], expectedStatus: ComputeRequest['status']) => Promise<ComputeRequest> }) {
  ...
  return <article className="admin-compute-detail" aria-label={`${request.company}的算力申请详情`}>
    ...
    <form className="admin-compute-status" onSubmit={updateStatus}><div><strong>处理状态</strong><small>状态只可按业务流程向前推进，关闭后不可恢复。</small></div>{availableStatuses.length ? <><label>下一状态<select aria-label="算力申请状态" value={nextStatus} disabled={updating} onChange={(event) => setNextStatus(event.target.value as ComputeRequest['status'])}><option value={request.status}>{computeStatusLabels[request.status]}（当前）</option>{availableStatuses.map((status) => <option value={status} key={status}>{computeStatusLabels[status]}</option>)}</select></label><button type="submit" disabled={updating || nextStatus === request.status}>{updating ? <CircleNotch className="spin" /> : <Check weight="bold" />}{nextStatus === request.status ? '选择下一状态' : `更新为${computeStatusLabels[nextStatus]}`}</button></> : <span className="admin-compute-final">该申请已结束</span>}{error && <div className="notice error" role="alert">{error}</div>}</form>
  </article>;
}
