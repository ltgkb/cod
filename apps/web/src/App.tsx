...
export function App() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([getCapabilities(), listModelCatalog(), resumeCodSession()]).then(async ([capabilityResult, catalogResult, sessionResult]) => {
      ...
      catch { if (mounted&&authGenerationRef.current===authGeneration) { logoutCod(); setAuthState('signed-out'); } }
    });
  ...
  const handleLogin = async (email: string, password: string) => {
    ...
    if(authGenerationRef.current!==authGeneration)throw new DOMException('Authentication attempt superseded','AbortError');
    persistCodSession(nextSession.token);sessionTokenRef.current=nextSession.token;setSession(nextSession);setAuthState('signed-in');setOverlay(resumeComputeAfterLogin ? 'compute' : null);setResumeComputeAfterLogin(false);
  };
