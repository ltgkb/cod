...
function LoginForm(props: LoginFormProps) {
  ...
  return <div className="login-form">
    ...
    {oidcEnabled && mode === 'login' && <button type="button" className="primary-button oidc-login-button" onClick={() => beginOidcLogin(capabilities?.authentication.oidcLoginUrl ?? undefined)}><ShieldCheck weight="bold" /> 使用 KAI 统一身份验证</button>}
    ...
    <div className="capability-summary"><span className={capabilities?.ai.mode === 'live' ? 'live' : 'demo'}>模型：{capabilities?.ai.mode === 'live' ? '已连接' : capabilities?.ai.mode === 'demo' ? '演示模式' : '待检测'}</span><span>认证：{registering ? verificationRequired?'邮箱 + 手机验证码':'邮箱密码（内测）' : oidcEnabled?'KAI 统一身份 / COD 密码':'COD 邮箱密码'}</span></div>
  </div>;
}
