...
export function App() {
  ...
  const handleSend = async () => {
    ...
    const canUseDesktopAgent = mode === 'code' && hasDesktopBridge() && Boolean(project.root);
    if (!canUseDesktopAgent && (!selectedSource?.callable || !selectedModelInfo)) { setNotice('当前模型源仅供查看目录，配置该源密钥后才能调用。'); return; }
    let task = activeTask;
    try {
      if (!task) {
      ...
      if (mode === 'code' && hasDesktopBridge() && project.root) appendMessage(task.id, { id: createClientId(), role: 'assistant', content: reply, mode: replyMode, createdAt: new Date().toISOString() });
  ...
  return <div className="app-shell">
    ...
    {overlay === 'account' && <Modal title="钱包与服务状态" onClose={() => setOverlay(null)}><div className="account-panel"><div className="account-balance"><small>可用预存余额</small><strong>¥ {(session.account.balanceCents / 100).toFixed(2)}</strong><span>{session.account.displayName} · {session.account.plan}</span></div><div className="service-grid"><span>模型网关<strong className={capabilities?.ai.mode === 'live' ? 'live' : 'demo'}>{capabilities?.ai.mode === 'live' ? '已连接' : capabilities?.ai.mode === 'demo' ? '演示模式' : '不可用'}</strong></span><span>当前源<strong className={selectedSource?.callable ? 'live' : 'demo'}>{selectedSource?.label ?? '未选择'}</strong></span><span>额度方式<strong>{capabilities?.payments.mode === 'pilot-credit' ? '试点额度' : '未接入'}</strong></span><span>支付方向<strong>{selectedSource?.paymentDirection ?? '未选择'}</strong></span></div>{capabilities?.payments.topupEnabled && <div className="topup-panel"><div><strong>预存试点额度</strong><small>仅用于本轮产品与计费闭环测试，不代表真实支付已到账。</small></div><div><button onClick={() => handleTopup(1000)}>+ ¥10</button><button onClick={() => handleTopup(5000)}>+ ¥50</button><button onClick={() => handleTopup(10000)}>+ ¥100</button></div></div>}<div className="ledger"><header><strong>最近流水</strong><button onClick={refreshWallet}><ArrowClockwise /> 刷新</button></header>{ledger.length ? ledger.map((entry) => <div key={entry.id}><span>{entry.type === 'usage' ? `${entry.model ?? '模型'} 用量` : '钱包预存'}<small>{entry.paymentDirection ?? entry.reference} · {formatTime(entry.createdAt)}</small></span><strong className={entry.amountCents < 0 ? 'negative' : 'positive'}>{entry.amountCents > 0 ? '+' : ''}¥ {(entry.amountCents / 100).toFixed(2)}</strong></div>) : <p>暂无流水</p>}</div><button className="secondary-button" onClick={handleLogout}><SignOut /> 退出登录</button></div></Modal>}
