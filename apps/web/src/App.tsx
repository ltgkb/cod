...
interface ChatMessage { id: string; role: 'user' | 'assistant' | 'comparison'; content: string; mode?: 'live' | 'demo'; sourceLabel?: string; model?: string; inputTokens?: number; outputTokens?: number; usageEstimated?: boolean; failed?: boolean; retryPrompt?: string; comparisonResults?: ComparisonResult[]; createdAt: string }
  ...
  const handleSend = async (requestedPrompt = prompt, requestedTask: RemoteTask | null = activeTask, requestedMode: WorkspaceMode = mode) => {
    ...
    try {
      ...
      if(comparisonRequest){
        ...
        appendMessage(task.id, { id: createClientId(), role: 'assistant', content: reply, mode: replyMode, sourceLabel: selectedSource.label, model: selectedModelInfo.id, inputTokens: result.inputTokens, outputTokens: result.outputTokens, usageEstimated: result.usageEstimated, createdAt: new Date().toISOString() });
        responseAppended = true;
      }
