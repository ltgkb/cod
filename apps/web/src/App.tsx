...
export function App() {
  ...
  const handleSend = async (requestedPrompt = prompt, requestedTask: RemoteTask | null = activeTask, requestedMode: WorkspaceMode = mode) => {
    ...
    try {
      ...
      if (!comparisonRequest&&requestedMode === 'code' && hasDesktopBridge() && project.root&&selectedSource&&selectedModelInfo) appendMessage(task.id, { id: createClientId(), role: 'assistant', content: reply, mode: replyMode, sourceLabel: selectedSource.label, model: selectedModelInfo.id, createdAt: new Date().toISOString() });
