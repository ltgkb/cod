...
export function App() {
  ...
  const keepNewConversationRef=useRef(false);
  ...
  const clearAuthenticatedUi = useCallback((message = '') => {
    ...
    activeRunRef.current=null;sendingRef.current=false;sessionTokenRef.current=null;keepNewConversationRef.current=false;
  ...
  useEffect(() => {
    const subscribe=window.codDesktop?.onDesktopPetOpenChat;
    if(!subscribe)return;
    return subscribe((incomingPrompt)=>{
      keepNewConversationRef.current=true;
      setMode('chat');
  ...
  const loadWorkspace = async (nextSession: CodSession, authGeneration:number) => {
    ...
    keepNewConversationRef.current=false;
    ...
    const sync = async () => {
      ...
      try {
        ...
        if(tasksResult.status==='fulfilled'){
          ...
          setTasks(nextTasks);
          setActiveTaskId((current)=>keepNewConversationRef.current&&current===null?null:preferredTaskId(nextTasks,current,ensuredDeviceId));
        }
  ...
  const selectWorkspaceMode=(nextMode:WorkspaceMode)=>{
    ...
    if(nextMode==='chat'&&activeTask&&activeTask.deviceId!==currentDeviceId){keepNewConversationRef.current=true;setActiveTaskId(null);setNotice('已开始当前设备的新对话；原任务仍保留在任务列表中。');}
    setMode(nextMode);
  };
  ...
  const handleCreateTask = async (event?: FormEvent) => {
    ...
    try {
      ...
      if(sessionTokenRef.current!==token)return;
      keepNewConversationRef.current=false;setTasks((current) => [task, ...current]); setActiveTaskId(task.id); setNewTaskTitle(''); setOverlay(null); setSidebarOpen(false); setNotice('任务已创建并同步到目标设备。');
  ...
  const handleRemoteTask = async () => {
    ...
    const token=session.token;
    try { const task = await createRemoteTask(token, title, targetDevice.id);if(sessionTokenRef.current!==token)return;keepNewConversationRef.current=false;setTasks((current) => [task, ...current]);setActiveTaskId(task.id);setNotice(`已发送到 ${targetDevice.name}。`); }
  ...
  const handleSend = async (requestedPrompt = prompt, requestedTask: RemoteTask | null = activeTask, requestedMode: WorkspaceMode = mode) => {
    ...
    try {
      ...
      if (!task) {
        ...
        keepNewConversationRef.current=false;setTasks((current) => [task!, ...current]); setActiveTaskId(task.id);
      }
  ...
  return <div className={`app-shell${inspectorOpen ? '' : ' inspector-hidden'}`}>
    ...
    {overlay === 'desktop-pet' && <Modal title="COD 桌面伙伴" onClose={() => setOverlay(null)}><div className="desktop-pet-panel"><div className="desktop-pet-state"><span className={desktopPetStatus?.running?'is-running':desktopPetStatus?.verified?'is-ready':'is-blocked'}><ChatCircleDots weight="fill" /></span><div><strong>{desktopPetStatus?.running?'正在运行':desktopPetStatus?.verified?'已内置':desktopPetStatus?.reason==='integrity-failed'?'内置资源校验失败':'当前版本未包含桌宠'}</strong><p>{desktopPetStatus?.running?`点击桌宠的“对话”可直接使用 ${selectedSource?.label??'当前模型'} · ${selectedModelInfo?.label??selectedModelInfo?.id??''}`:desktopPetStatus?.verified?`桌宠 ${desktopPetStatus.version} 已随 COD Desktop 内置，无需另行下载安装。`:desktopPetStatus?.reason==='integrity-failed'?'内置桌宠资源与已审计版本不一致，COD 已拒绝运行。':'请更新到包含内置桌宠的 COD Desktop。'}</p></div></div><div className="desktop-pet-audit"><ShieldCheck weight="bold" /><div><strong>内置资源完整性已校验</strong><p>桌宠只从 COD 安装目录加载，不能替换为未知程序；默认不自启，由你在这里启动或停止。</p></div></div><div className="desktop-pet-actions"><button type="button" className="secondary-button" disabled={desktopPetBusy} onClick={() => void refreshDesktopPetStatus()}>{desktopPetBusy?<CircleNotch className="spin"/>:<ArrowClockwise/>}刷新状态</button>{desktopPetStatus?.running?<button type="button" className="primary-button" disabled={desktopPetBusy} onClick={() => void handleDesktopPetStop()}><Stop weight="fill"/>停止桌宠</button>:<button type="button" className="primary-button" disabled={desktopPetBusy||!desktopPetStatus?.verified||!session||!selectedSource?.callable||!selectedModelInfo} onClick={() => void handleDesktopPetLaunch()}><Play weight="fill"/>启动内置桌宠</button>}</div>{!session&&<p className="desktop-pet-help">登录 COD 并选择模型后即可启动；桌宠不会保存或获得你的登录密码。</p>}</div></Modal>}
