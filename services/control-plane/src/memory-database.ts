...
import type { AuditEntry, CodDatabase, LedgerEntry, Principal, SyncedTask, TaskEvent, TopupRequest } from './database.js';
...
export class MemoryDatabase implements CodDatabase {
  ...
  async topup(p: Principal, request: TopupRequest) {
    ...
    const entry:LedgerEntry={id:randomUUID(),type:'topup',amountCents:request.amountCents,createdAt:new Date().toISOString(),reference:`${request.channel}:${request.idempotencyKey}`};
  ...
  async recordUsage(p: Principal,event:UsageEvent) {
    ...
    const entry:LedgerEntry={id:randomUUID(),type:'usage',amountCents:-event.costCents,createdAt:new Date().toISOString(),reference:`${event.model}:${event.taskId}`};
  ...
  async settleUsage(p:Principal,id:string,event:UsageEvent){const state=this.state(p);const existing=state.idempotency.get(event.idempotencyKey);if(existing)return existing;const reservation=state.reservations.get(id);if(!reservation||reservation.status!=='reserved')throw new HttpError('Usage reservation not found',409,'reservation_not_found');const delta=reservation.amountCents-event.costCents;if(delta>=0)state.account={...state.account,balanceCents:state.account.balanceCents+delta};else{if(state.account.balanceCents < -delta)throw new HttpError('Insufficient balance',402,'insufficient_balance');state.account={...state.account,balanceCents:state.account.balanceCents+delta};}const entry:LedgerEntry={id:randomUUID(),type:'usage',amountCents:-event.costCents,createdAt:new Date().toISOString(),reference:`${event.model}:${event.taskId}`};state.ledger.unshift(entry);state.idempotency.set(event.idempotencyKey,entry);reservation.status='settled';return entry;}
  ...
  async listDevices(p:Principal){return [...this.state(p).devices.values()];}
  async registerDevice(p:Principal,input:Pick<DeviceRecord,'name'|'platform'>){if(!input.name?.trim())throw new HttpError('Device name is required',400,'invalid_device');const state=this.state(p);const device:DeviceRecord={id:randomUUID(),name:input.name.trim().slice(0,100),platform:input.platform,status:'online',lastSeenAt:new Date().toISOString()};state.devices.set(device.id,device);this.append(state,'device.registered',device.id,device);return device;}
  async heartbeat(p:Principal,id:string){const state=this.state(p);const current=state.devices.get(id);if(!current)throw new HttpError('Device not found',404,'device_not_found');const device={...current,status:'online' as const,lastSeenAt:new Date().toISOString()};state.devices.set(id,device);this.append(state,'device.heartbeat',id,device);return device;}
  async listTasks(p:Principal){return [...this.state(p).tasks.values()];}
  async createTask(p:Principal,input:Pick<SyncedTask,'title'|'deviceId'>){const state=this.state(p);if(!input.title?.trim())throw new HttpError('Task title is required',400,'invalid_task');if(!state.devices.has(input.deviceId))throw new HttpError('Device not found',404,'device_not_found');const task:SyncedTask={id:randomUUID(),title:input.title.trim().slice(0,500),deviceId:input.deviceId,status:'draft',updatedAt:new Date().toISOString(),version:1};state.tasks.set(task.id,task);this.append(state,'task.created',task.id,task);return task;}
  async updateTask(p:Principal,id:string,status:TaskStatus,version:number){const state=this.state(p);const current=state.tasks.get(id);if(!current)throw new HttpError('Task not found',404,'task_not_found');if(current.version!==version)throw new HttpError('Task version conflict',409,'version_conflict');const task={...current,status,version:current.version+1,updatedAt:new Date().toISOString()};state.tasks.set(id,task);this.append(state,'task.updated',id,task);return task;}
