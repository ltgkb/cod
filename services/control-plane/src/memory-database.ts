...
export class MemoryDatabase implements CodDatabase {
  ...
  async initialize() { await this.reapExpiredUsageReservations();await this.cleanupRegistrationChallenges(new Date());if(!this.reservationReaperTimer){this.reservationReaperTimer=setInterval(()=>{if(this.reservationReaperRun)return;const run=this.reapExpiredUsageReservations();this.reservationReaperRun=run;void run.catch((error:unknown)=>console.error(JSON.stringify({level:'error',event:'usage.reservations.reap_failed',error:error instanceof Error?error.message:String(error)}))).finally(()=>{if(this.reservationReaperRun===run)this.reservationReaperRun=null;});},USAGE_RESERVATION_REAP_INTERVAL_MS);this.reservationReaperTimer.unref();} }
  ...
  async startPhoneRegistration(input:StartPhoneRegistrationInput){
    ...
    if([...this.identities.values()].some((identity)=>identity.phoneE164===phone))throw new HttpError('该手机号已经注册',409,'phone_registered');
    for(const other of this.registrationChallenges.values())if(other.id!==item.id&&other.status==='pending'&&other.phone===phone&&other.expiresAt<=now){other.status='superseded';other.emailCodeHash=null;other.phoneCodeHash=null;}
    if([...this.registrationChallenges.values()].some((other)=>other.id!==item.id&&other.status==='pending'&&other.phone===phone&&other.expiresAt>now))throw new HttpError('该手机号正在用于其他注册验证',409,'phone_registration_pending');if(item.phoneVerifiedAt)throw new HttpError('手机号已经完成验证',409,'registration_phone_already_verified');
  ...
  async cleanupRegistrationChallenges(now:Date,limit=1000):Promise<number>{
    const maximum=Math.max(1,Math.min(1000,Math.trunc(limit)||1));const timestamp=now.getTime();const terminalCutoff=timestamp-24*60*60*1000;let affected=0;
    for(const [id,item] of this.registrationChallenges){
      if(affected>=maximum)break;
      if(item.status==='pending'&&item.expiresAt<=timestamp){item.status='superseded';item.emailCodeHash=null;item.phoneCodeHash=null;affected+=1;continue;}
      if(item.status==='consumed'&&item.replayUntil!==null&&item.replayUntil<=timestamp||item.status!=='pending'&&item.status!=='consumed'&&item.expiresAt<=terminalCutoff){this.registrationChallenges.delete(id);affected+=1;}
    }
    return affected;
  }
  ...
  async claimChatRequest(p:Principal,requestKey:string,fingerprint:string,executionId?:string):Promise<ChatRequestClaim>{
    ...
    if(executionId&&!/^[a-f0-9]{8}-(?:[a-f0-9]{4}-){3}[a-f0-9]{12}$/i.test(executionId))throw new HttpError('Task execution is invalid',400,'invalid_task_execution');
    if(!existing){state.chatRequests.set(requestKey,{fingerprint,executionId:executionId??null,status:'pending',responsePayload:null,expiresAt:now+60*60*1000});return{state:'claimed'};}
    ...
    if(existing.executionId!==(executionId??null))throw new HttpError('Request ID belongs to another task execution',409,'idempotency_conflict');
    ...
    state.chatRequests.set(requestKey,{fingerprint,executionId:executionId??null,status:'pending',responsePayload:null,expiresAt:now+60*60*1000});return{state:'claimed'};
  }
  async failChatRequest(p:Principal,requestKey:string,fingerprint:string,executionId?:string){
    const state=this.state(p);const existing=state.chatRequests.get(requestKey);if(!existing||existing.fingerprint!==fingerprint||existing.executionId!==(executionId??null)||existing.status!=='pending')return;
  ...
  async settleUsage(p:Principal,id:string,event:UsageEvent,completion?:ChatRequestCompletion,executionId?:string){
    validateUsageEvent(event);const billedEvent=billedUsageEvent(p,event);
    ...
    if(completion){if(event.idempotencyKey!==`chat:${completion.requestKey}:${completion.fingerprint}`)throw new HttpError('Chat settlement key is invalid',400,'invalid_idempotency_key');if(Buffer.byteLength(JSON.stringify(completion.responsePayload),'utf8')>CHAT_RESPONSE_CACHE_MAX_BYTES)throw new HttpError('Model response is too large to cache safely',502,'chat_response_cache_too_large');if(!chatRequest||chatRequest.fingerprint!==completion.fingerprint||chatRequest.executionId!==(completion.executionId??null))throw new HttpError('Chat request claim was not found',409,'chat_request_not_claimed');if(chatRequest.status==='failed')throw new HttpError('Chat request is no longer pending',409,'chat_request_not_pending');}
    ...
    const existing=state.idempotency.get(event.idempotencyKey);if(existing){if(!usageMatchesLedger(existing,billedEvent))throw new HttpError('Idempotency key was already used with different usage parameters',409,'idempotency_conflict');if(reservation?.status==='reserved')this.releaseReservation(state,reservation);if(completion&&chatRequest?.status==='pending')state.chatRequests.set(completion.requestKey,{fingerprint:completion.fingerprint,executionId:completion.executionId??null,status:'complete',responsePayload:cachedResponse!,expiresAt:Date.now()+24*60*60*1000});return existing;}
  ...
  async quoteAdminComputeRequest(p:Principal,id:string,quote:ComputeQuoteInput,expectedStatus:ComputeRequestStatus){
    requireAdmin(p);validateComputeRequestId(id);validateComputeRequestStatus(expectedStatus);const adminState=this.state(p);
    for(const state of this.users.values()){
      const current=state.computeRequests.get(id);if(!current)continue;
      ...
      const now=new Date().toISOString();const request:ComputeRequest={...current,quote:{...quote,currency:'CNY',cardHoursMilli:quote.cardHoursMilli??null,createdAt:now},quoteDecision:null,quoteDecisionAt:null,status:'quoted',updatedAt:now};
      state.computeRequests.set(id,request);adminState.audit.unshift({id:randomUUID(),action:'compute.request.admin.quote',entityType:'compute_request',entityId:id,data:{previousStatus:current.status,status:request.status,amountCents:quote.amountCents,cardHoursMilli:quote.cardHoursMilli??null,validUntil:quote.validUntil},createdAt:now});
      return{request:{...request},previousStatus:current.status,changed:true};
    }
    throw new HttpError('Compute request not found',404,'compute_request_not_found');
  }
  async decideComputeRequestQuote(p:Principal,id:string,decision:ComputeQuoteDecision,expectedStatus:ComputeRequestStatus){
    ...
    const state=this.state(p);const current=state.computeRequests.get(id);if(!current)throw new HttpError('Compute request not found',404,'compute_request_not_found');
    if(current.status!==expectedStatus)throw new HttpError('Compute request status changed; reload and confirm the latest state',409,'compute_request_status_conflict');
    ...
    const now=new Date().toISOString();const status=decision==='accepted'?'approved':'closed';const request:ComputeRequest={...current,quoteDecision:decision,quoteDecisionAt:now,status,updatedAt:now};state.computeRequests.set(id,request);
    state.audit.unshift({id:randomUUID(),action:'compute.request.quote.decision',entityType:'compute_request',entityId:id,data:{previousStatus:current.status,status,decision},createdAt:now});
    return{request:{...request},previousStatus:current.status,changed:true};
  }

  async listDevices(p:Principal){const state=this.state(p);return[...state.devices.values()].map((device)=>{if(Date.now()-new Date(device.lastSeenAt).getTime()<=DEVICE_OFFLINE_AFTER_MS)return device;const offline={...device,status:'offline' as const};state.devices.set(device.id,offline);return offline;});}
  ...
  async heartbeat(p:Principal,id:string,taskLease?:TaskLeaseHeartbeat){const state=this.state(p);const current=state.devices.get(id);if(!current)throw new HttpError('Device not found',404,'device_not_found');if(taskLease){validateTaskExecutionCredential(taskLease);const execution=state.taskExecutions.get(taskLease.taskId);const task=state.tasks.get(taskLease.taskId);if(!execution||!task||task.deviceId!==id||!['running','waiting'].includes(task.status)||execution.executionId!==taskLease.executionId||execution.leaseTokenHash!==hashTaskLeaseToken(taskLease.leaseToken)||execution.leaseExpiresAt<=Date.now())throw new HttpError('Task execution lease is no longer active',409,'task_lease_expired');state.taskExecutions.set(taskLease.taskId,{...execution,leaseExpiresAt:Date.now()+TASK_LEASE_DURATION_MS});}const device={...current,status:'online' as const,lastSeenAt:new Date().toISOString()};state.devices.set(id,device);return device;}
  async listTasks(p:Principal){const state=this.state(p);this.expireTaskLeases(state);return[...state.tasks.values()];}
  async getTask(p:Principal,id:string){const state=this.state(p);this.expireTaskLeases(state,id);const task=state.tasks.get(id);if(!task)throw new HttpError('Task not found',404,'task_not_found');return task;}
  async createTask(p:Principal,input:Pick<SyncedTask,'title'|'deviceId'>){const state=this.state(p);if(!input||typeof input!=='object'||!input.title?.trim())throw new HttpError('Task title is required',400,'invalid_task');const device=state.devices.get(input.deviceId);if(!device)throw new HttpError('Device not found',404,'device_not_found');if(device.status!=='online'||Date.now()-new Date(device.lastSeenAt).getTime()>DEVICE_OFFLINE_AFTER_MS)throw new HttpError('Device is offline',409,'device_offline');const task:SyncedTask={id:randomUUID(),title:input.title.trim().slice(0,500),deviceId:input.deviceId,status:'draft',updatedAt:new Date().toISOString(),version:1,result:null,error:null};state.tasks.set(task.id,task);this.append(state,'task.created',task.id,task);return task;}
  async claimTask(p:Principal,id:string,version:number,claim:TaskExecutionClaimCredential):Promise<TaskExecutionClaim>{const state=this.state(p);validateTaskExecutionClaimCredential(claim);this.expireTaskLeases(state,id);const current=state.tasks.get(id);if(!current)throw new HttpError('Task not found',404,'task_not_found');const active=state.taskExecutions.get(id);if(current.status==='running'||current.status==='waiting'){if(!active||active.claimIdHash!==hashTaskLeaseToken(claim.claimId)||active.leaseTokenHash!==hashTaskLeaseToken(claim.leaseToken)||active.leaseExpiresAt<=Date.now())throw new HttpError('Task already has an active execution',409,'task_already_running');return{task:current,executionId:active.executionId,leaseToken:claim.leaseToken,leaseExpiresAt:new Date(active.leaseExpiresAt).toISOString(),replayed:true};}if(current.version!==version)throw new HttpError('Task version conflict',409,'version_conflict');validateTaskTransition(current.status,'running');const device=state.devices.get(current.deviceId);if(!device||device.status!=='online'||Date.now()-new Date(device.lastSeenAt).getTime()>DEVICE_OFFLINE_AFTER_MS)throw new HttpError('Device is offline',409,'device_offline');const executionId=randomUUID();const leaseExpiresAt=Date.now()+TASK_LEASE_DURATION_MS;const task={...current,status:'running' as const,result:null,error:null,version:current.version+1,updatedAt:new Date().toISOString()};state.tasks.set(id,task);state.taskExecutions.set(id,{executionId,claimIdHash:hashTaskLeaseToken(claim.claimId),leaseTokenHash:hashTaskLeaseToken(claim.leaseToken),leaseExpiresAt});this.append(state,'task.updated',id,task);state.audit.unshift({id:randomUUID(),action:'task.execution.claim',entityType:'task',entityId:id,data:{executionId,leaseExpiresAt:new Date(leaseExpiresAt).toISOString()},createdAt:new Date().toISOString()});return{task,executionId,leaseToken:claim.leaseToken,leaseExpiresAt:new Date(leaseExpiresAt).toISOString(),replayed:false};}
  async assertTaskExecution(p:Principal,id:string,executionId:string){const state=this.state(p);this.expireTaskLeases(state,id);return this.assertExecutionState(state,id,executionId);}
  async assertTaskLease(p:Principal,id:string,execution:TaskExecutionCredential){validateTaskExecutionCredential(execution);const state=this.state(p);this.expireTaskLeases(state,id);const task=this.assertExecutionState(state,id,execution.executionId);const active=state.taskExecutions.get(id)!;if(active.leaseTokenHash!==hashTaskLeaseToken(execution.leaseToken))throw new HttpError('Task execution lease is no longer active',409,'task_lease_expired');return task;}
  async renewTaskExecution(p:Principal,id:string,executionId:string){const state=this.state(p);this.assertExecutionState(state,id,executionId);const active=state.taskExecutions.get(id)!;state.taskExecutions.set(id,{...active,leaseExpiresAt:Date.now()+TASK_LEASE_DURATION_MS});}
  async updateTask(p:Principal,id:string,status:TaskStatus,version:number,outcome:TaskOutcome={},execution?:TaskExecutionCredential){const state=this.state(p);this.expireTaskLeases(state,id);const current=state.tasks.get(id);if(!current)throw new HttpError('Task not found',404,'task_not_found');if(current.version!==version)throw new HttpError('Task version conflict',409,'version_conflict');if(status==='running'&&current.status!=='running'&&current.status!=='waiting')throw new HttpError('Running tasks must claim a new execution lease',409,'task_claim_required');validateTaskTransition(current.status,status);if(outcome.result!==undefined&&outcome.result!==null&&typeof outcome.result!=='string')throw new HttpError('Task result is invalid',400,'invalid_task_result');if(outcome.error!==undefined&&outcome.error!==null&&typeof outcome.error!=='string')throw new HttpError('Task error is invalid',400,'invalid_task_error');if((current.status==='running'||current.status==='waiting')&&status!=='cancelled'){if(!execution)throw new HttpError('Task execution lease is required',409,'task_lease_required');validateTaskExecutionCredential(execution);const active=state.taskExecutions.get(id);if(!active||active.executionId!==execution.executionId||active.leaseTokenHash!==hashTaskLeaseToken(execution.leaseToken)||active.leaseExpiresAt<=Date.now())throw new HttpError('Task execution lease is no longer active',409,'task_lease_expired');}if(outcome.result!==undefined&&outcome.result!==null&&outcome.result.length>50_000)throw new HttpError('Task result is too large',400,'task_result_too_large');if(outcome.error!==undefined&&outcome.error!==null&&outcome.error.length>5_000)throw new HttpError('Task error is too large',400,'task_error_too_large');if(current.status===status&&outcome.result===undefined&&outcome.error===undefined)return current;let result=outcome.result===undefined?current.result:outcome.result;let error=outcome.error===undefined?current.error:outcome.error;if(current.status==='waiting'&&status==='running'){result=null;error=null;}if(status==='complete')error=null;if(status==='failed')result=null;if(status==='cancelled'){result=null;error=null;}validateTaskOutcome(status,result,error);const task={...current,status,result,error,version:current.version+1,updatedAt:new Date().toISOString()};state.tasks.set(id,task);if(status==='complete'||status==='failed'||status==='cancelled')state.taskExecutions.delete(id);this.append(state,'task.updated',id,task);return task;}
  ...
  private state(p:Principal){const key=this.key(p);let state=this.users.get(key);if(!state){const now=new Date();const trial:CreditGrant={id:randomUUID(),packId:'trial',name:'新用户试用金',originalCents:1000,remainingCents:1000,purchasedAt:now.toISOString(),expiresAt:new Date(now.getTime()+30*24*60*60*1000).toISOString(),status:'active'};const trialEntry:LedgerEntry={id:randomUUID(),type:'trial_credit',amountCents:1000,walletAmountCents:0,creditAmountCents:1000,createdAt:now.toISOString(),reference:'新用户试用金',sourceId:null,model:null,paymentDirection:'平台赠送 → COD 使用额度'};state={account:{userId:p.userId,displayName:p.email.split('@')[0],balanceCents:0,currency:'CNY',plan:p.role==='admin'?'team':'developer',role:p.role,billingExempt:p.role==='admin'},ledger:[trialEntry],idempotency:new Map(),devices:new Map(),tasks:new Map(),taskExecutions:new Map(),events:[],audit:[],reservations:new Map(),chatRequests:new Map(),creditGrants:new Map([[trial.id,trial]]),creditPackIdempotency:new Map(),paymentOrders:new Map(),paymentIdempotency:new Map(),computeRequests:new Map(),computeIdempotency:new Map()};this.users.set(key,state);}else if(state.account.role!==p.role||state.account.billingExempt!==(p.role==='admin'))state.account={...state.account,role:p.role,billingExempt:p.role==='admin'};return state;}
  private expireTaskLeases(state:UserState,taskId?:string){for(const [id,current]of state.tasks){if(taskId&&id!==taskId)continue;if(current.status!=='running'&&current.status!=='waiting')continue;const execution=state.taskExecutions.get(id);if(execution&&execution.leaseExpiresAt>Date.now())continue;const legacy=!execution;const task={...current,status:'failed' as const,result:null,error:legacy?LEGACY_TASK_INTERRUPTED_ERROR:TASK_LEASE_EXPIRED_ERROR,version:current.version+1,updatedAt:new Date().toISOString()};state.tasks.set(id,task);state.taskExecutions.delete(id);this.append(state,'task.updated',id,task);state.audit.unshift({id:randomUUID(),action:'task.execution.interrupted',entityType:'task',entityId:id,data:{reason:legacy?'legacy_missing_lease':'lease_expired',executionId:execution?.executionId??null},createdAt:new Date().toISOString()});}}
  private assertExecutionState(state:UserState,taskId:string,executionId:string):SyncedTask{const task=state.tasks.get(taskId);const execution=state.taskExecutions.get(taskId);if(!task)throw new HttpError('Task not found',404,'task_not_found');if(!execution||execution.executionId!==executionId||execution.leaseExpiresAt<=Date.now()||(task.status!=='running'&&task.status!=='waiting'))throw new HttpError('Task execution lease is no longer active',409,'task_lease_expired');return task;}
