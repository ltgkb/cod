...
describe('synchronization database contract', () => {
  ...
  it('rejects malformed task outcomes as client errors', async () => {
    ...
    const claim = await database.claimTask(principal, task.id, task.version, claimCredential('V'));
    await expect(database.updateTask(principal, task.id, 'complete', claim.task.version, { result: 42 as unknown as string }, claim)).rejects.toMatchObject({ status: 400, code: 'invalid_task_result' });
    await expect(database.updateTask(principal, task.id, 'failed', claim.task.version, { error: {} as unknown as string }, claim)).rejects.toMatchObject({ status: 400, code: 'invalid_task_error' });
  });
