...
- Internal beta registration may be explicitly opened with `COD_REGISTRATION_ENABLED=true` and `COD_REGISTRATION_VERIFICATION_REQUIRED=false`; this accepts only allowlisted email domains, enforces password strength and rate limits, and does not claim that an email or phone was verified. Before public launch, switch verification back on and configure email OTP, SMS OTP, Turnstile, the public registration URL, and the 32-byte registration HMAC key. Existing accounts and the one-time legacy migration remain available independently.
