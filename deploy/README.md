...
- Public registration stays disabled until email ownership verification is available; otherwise an arbitrary `@kai.com` string could claim an account and its 30-day trial credit. Existing accounts can still sign in normally, and only the configured passwordless pilot identity may use the one-time legacy access-code migration.
